...
absent) rather than fabricating a URL — see §16. **2026-09-15:** the guard on
`work.index.tsx`'s hover-stage and `clients.tsx`'s roster row used to just
omit the link with nothing in its place; both now show the same "Print /
brand piece" label `work.$slug.tsx` already used, so Ozone reads as a
deliberate distinction everywhere, not a silently missing link.
