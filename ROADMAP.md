...
0. **Live-browser-verify the 2026-09-03 homepage premium pass.** (Was:
   ...
   viewport.
1. **Decide a deployment target and add its config.** Nothing is committed yet — no CI workflow, no platform config. Repo is frontend-only now (static/SSR host — Amplify/Vercel/Netlify); the Django backend moved to `../maco-backend`, out of scope until it's reintroduced.
2. **Full responsive + accessibility verification pass** — real devices, `forced-colors`, screen reader, Lighthouse numbers. See `PROJECT_STATUS.md` "Not yet verified".
3. **Decide whether a test suite is worth adding.** Currently zero tests; `build` + `lint` are the only gates. Fine for a marketing site with no business logic on the frontend.
